...
task("setValue02")
    .addParam("value02", "The value to set", undefined, types.int)
    .setAction(async (taskArgs, hre) => {
        const { network } = hre;
        const [dev] = await hre.ethers.getSigners();
        const addressList = readAddressList();
        ...
        const tx = await myContract.setValue02(taskArgs.value02);
        console.log("tx: ", await tx.wait());

        const currentValue02 = await myContract.value02();
        console.log("currentValue: ", currentValue02.toString());
    });